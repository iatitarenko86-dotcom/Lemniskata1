...
def handle_percentage_tasks(bot, message, user_data):
    ...
    if current_submodule in ['easy_simulator', 'medium_simulator', 'hard_simulator', 'random_simulator']:
        ...
        системные_кнопки = [
            ...
            '📱 Пример 1: Скидка на телефон', '🧈 Пример 2: Скидка пенсионерам',
            '📚 Пример 3: Двойное снижение цены', '💰 Пример 4: Налог на доходы',
            '🌳 Пример 5: Деревья в парке'
        ]
    ...
    elif text == '📱 Пример 1: Скидка на телефон':
        send_example_1(bot, user_id)

    elif text == '🧈 Пример 2: Скидка пенсионерам':
        send_example_2(bot, user_id)

    elif text == '📚 Пример 3: Двойное снижение цены':
        send_example_3(bot, user_id)

    elif text == '💰 Пример 4: Налог на доходы':
        send_example_4(bot, user_id)

    elif text == '🌳 Пример 5: Деревья в парке':
